...
bool embed_DAG(
    ...
    std::cout <<"Incoming node options :";
    for(auto x : nodes){
        std::cout << x.options.size() << ",";
    } std::cout<<std::endl;
    ...
    //TODO: ~smart ordering~ <---- need to do this + ordring of node options
    std::cout <<"Number of incoming nodes to embed: " << nodes.size() << std::endl;
	for (uint32_t i = 0; i < nodes.size(); ++i) {
		select_order.emplace_back(i);
	}
    // isn't the incoming order atleast sorted by 'y', maybe don't move them around ?
    ...
    std::cout <<"Node order:";
    for(auto x : select_order){
        std::cout <<x <<"[" << nodes[x].options.size() <<"], ";
    }
    ...
    auto expand_state = [&queue_state, &select_order, &add_left_of, &nodes, &edges, &depth](State const &state, DAGCost const &cost) {
		assert(state.step < select_order.size());
		uint32_t n = select_order[state.step];
		DAGNode const &node = nodes[n];
        int tried_options = 0;
		for (auto const &option : node.options) {
            //if( &option - &node.options[0] > 10) break; // only try some options ? todo need a better way to order options, right? but still legal correct
			State next = state;
			next.step += 1;
            ...
            tried_options++;
            if(depth > 0 && tried_options > depth) break; // maybe until i figure a nice way to do this, should keep some sort of branching factor in the ui
