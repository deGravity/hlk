...
namespace vkmp {
	...
	struct BedNeedle {
		...
		// not very useful but helps in keeping a set
		bool operator<(BedNeedle const& o) const {
			return needle == o.needle ? bed < o.bed : needle < o.needle;
		}
...
template< >
struct std::hash< vkmp::BedNeedle > {
	size_t operator()(vkmp::BedNeedle const &bn) const {
