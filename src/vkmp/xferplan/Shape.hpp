...
struct Shape {
	...
	void append_to_beds(C const &data, T const &gap, std::vector< T > *front_, std::vector< T > *back_) const {
		...
		if (on_back && on_front) {
            ...
            while (front.size() + front_add < back.size() + back_add) front.push_back(gap);
            while (back.size() + back_add < front.size() + front_add) back.push_back(gap);
			if (nibbles & BackLeft) {
		...
		for (uint32_t i = 0; i < on_front; ++i) {
            front.push_back(data[(start + i) % data.size()]);
		}
		for (uint32_t i = on_back - 1; i < on_back; --i) {
            back.push_back(data[(start + on_front + i) % data.size()]);
		}
