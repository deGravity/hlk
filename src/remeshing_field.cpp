...
#include "directional_ext.h"  // adds dual_cycles direction-constraints overload + index_prescription linf-vector overloads
...
void RemeshingMenu::update_raw_field() {
    ...
    if (!cfaces.empty()) {
        ...
        if (do_matching) {
            ...
            Eigen::VectorXi unused_matching;
            Eigen::VectorXd unused_effort;
            Meshing::comb_field_from_connection(V, F, EV, EF, FE, rawField, combedField, unused_matching, unused_effort);
